...
library(ggh4x) #for tuning the scales of facets
```
...
```{r}
#define primary household member based on most receipts contributed

#for each household_ID in survey_data_all_wide, define (summurise) MAX n_receipts_user and their ID, who contributed the most receipts
survey_data_all_wide <- 
  survey_data_all_wide %>%
  group_by(household_ID) %>%
  mutate(
    status = case_when(
      n_receipts_user == max(n_receipts_user) ~ "Primary participant",
      TRUE ~ "Secondary participant"
    ),
    .after = "user_ID"
  ) %>%
  ungroup()
...
demographics <- 
  survey_data_all_wide %>%
  select(status,age:diet) %>% #,apps_used) %>%
  # mutate(apps_used = as.factor(apps_used)) %>%
  mutate(across(where(is.factor), ~ fct_drop(., NULL)))

names(demographics) <- 
  str_to_sentence(names(demographics))
  ...
  tbl_summary(
    ...
    label = 
      list(
        ...
        # Apps_used ~ "Used apps to calculate carbon footprint before",
        Does_groceries ~ "Who does the groceries"
      )
  ) %>%
  #add a total/overall count
  add_overall() %>%
  # adding spanning header for participant roles
  modify_spanning_header(c("stat_1", "stat_2") ~ "Participant role") %>% 
  modify_footnote(update = everything() ~ NA) %>%
  modify_footnote(
    c("stat_1","stat_2") ~ "The primary, or dominant, participant is the household participant who contributed the most receipts. The secondary participant is the other household participant."
    )
...
demographics %>% 
  as_gt() %>% 
  gtsave(here("output","tables","demographics.docx"))

#add manual footnote to 'used apps' row in html
#one participant used: WWF's my footprint app, Klima and myplannetapp
#one participant used: Co2.myclimate.org; ourworldindata; for travel the trip providers often have an estimate


```

```{r food acquisition}
#table of food acquisition sources

shares <- survey_data_all_wide %>% 
  select(share_store:`share_out-of-home`) %>%
  pivot_longer(cols = share_store:`share_out-of-home`, names_to = "Source", values_to = "frequency") %>%
  group_by(Source) %>%
  count(frequency) %>%
  mutate(ratio = n/sum(n)) %>%
  select(-n) %>%
  #rename source names by removing "share_" and "_"
  mutate(
    Source = str_remove_all(Source,"share_"),
    Source = str_replace_all(Source,"_"," ")
  ) %>%
  
  # pivot_wider(names_from = frequency, values_from = c(ratio)) %>%
  pivot_wider(names_from = Source, values_from = c(ratio)) %>%
  #replace NA by 0
  mutate(
    across(
      where(is.numeric),
...
#reorder columns
shares %<>%
  relocate(store,online,`meal box`,`veggie box`,`farm store`,`out-of-home`)
  
#create GT table
shares %<>%
  gt(rowname_col = "frequency") %>% #rowname_col = "Source") %>%
  # tab_spanner(
  #   label = "Proportion of food acquired from different sources",
  #   columns = `none/very little`:`almost all`
  # ) %>%
  fmt_percent(
...
#save table
shares %>% 
  gtsave(here("output","tables","food_acquisition.docx"))
...
survey_data_all %<>% 
  mutate(
    survey_id = case_match(survey_id, "T1 (start)" ~ "Pre Study", "T2 (end)" ~ "Post Study")
  ) %>%
  mutate(
    survey_id = factor(survey_id, levels = c("Pre Study","Post Study"))
  )
```
...
survey_data_all %>% 
  ...
  #order the concepts
  mutate(concept = factor(concept, levels = c("Environmental attitude","Self-efficacy","Intention","Spillover")))
...
p <- survey_data_all %>% 
  ...
  #calculate the mean and standard deviation for each concept
  group_by(concept, survey_id) %>%
  summarise(
    n=n(),
    mean = mean(response),
    sd = sd(response)
  ...
  #create the boxplot
  ggplot(aes(x=concept, y=mean, fill = survey_id)) + 
  geom_bar(stat = "identity", position=position_dodge(width=0.9), alpha = 0.8) + 
  geom_errorbar( aes(x=concept, ymin=mean-ic, ymax=mean+ic), width=0.4, alpha=0.9, size=.7, position=position_dodge(width=0.9), alpha = 0.5) +
...
# ggsave(here("output","plots","barcharts_concepts_survey.png"), width = 4.5, height = 4)

```

```{r barcharts all composites}

p <- survey_data_all %>% 
  
  #select the data
  select(survey_id, env_attitude, knowledge_score, SelfEff, intention, spillover) %>% 
  ...
  #order the concepts
  mutate(concept = factor(concept, levels = c("Environmental attitude","Knowledge score","Self-efficacy","Intention","Spillover"))) %>%
...
ggsave(here("output","plots","barcharts_concepts_survey.png"), width = 6)
...
survey_data_all_wide %>%
  mutate(
    knowledge_gain = knowledge_score_T2 - knowledge_score_T1, 
    # .keep = "none"
    .keep = "used"
  ) %>% 
  stat.desc(norm = T) 
...
# t(21) = 4.1, p < 0.001, d = 0.83 (large effect)
```
...
survey_data_all_wide %>%
  mutate(efficacy_gain = SelfEff_T2 - SelfEff_T1, .keep = "used") %>% 
  stat.desc(norm = T) 
#normally distributed
...
# t(21) = 4.9, p < 0.001, d = 0.80 (large effect)
```
...
# t(21) = 7.3, p < 0.001, d = 1.57 (large effect)
...
# t(21) = 3.2, p < 0.01, d = .68 (moderate effect)
```
...
stats_data <-
  ...
  #select the data
  select(user_ID, survey_id, env_attitude, knowledge_score, SelfEff, intention, spillover) %>% 
  
  #long format for ggplot
  pivot_longer(
    cols = c(env_attitude, knowledge_score, SelfEff, intention, spillover), 
    names_to = "concept", values_to = "response"
  ) %>%
  ...
  #replace NA's in response column with the value 4 (the population mean that we compare against in this case) 
  mutate(response = replace_na(response,4)) %>%
  
  #create different columns for pre- and post-study
    pivot_wider(
  ...
  group_by(concept) %>%
  mutate(
    difference = `Post Study` - `Pre Study`,
    # .keep = "unused"
  )
...
```{r combine t-tests}
tests <-
#combine test results in a table
cbind(
  
  #t-test attitudes
  with(survey_data_all_wide, 
     t.test(env_attitude_T2, env_attitude_T1, paired = T, alternative = "greater")
     ) %>% 
  # broom::tidy()
  unlist() %>%
  as.data.frame() %>%
  rename(`Environmental attitude` = 1),
  ...
  unlist() %>%
  as.data.frame() %>%
  rename(`Knowledge score` = 1),
  
    #t-test self efficacy
  with(survey_data_all_wide, 
     t.test(SelfEff_T2,SelfEff_T1, paired = T, alternative = "greater")
     ) %>% 
  # broom::tidy()
...
tests %<>%
  pivot_longer(cols = -measure, names_to = "concept", values_to = "value")
...
tests %<>%
  mutate(
    across(
      c(statistic.t, parameter.df, p.value), 
      as.numeric
    ))
```

```{r combine effect sizes}
effect_sizes <-
  bind_rows(
    survey_data_all %>%  
      cohens_d(env_attitude ~ survey_id, paired = TRUE),
...
effect_sizes %<>%
  select(concept, effsize)

effect_sizes %<>% 
  mutate(
    effsize = round(effsize,2) %>% abs()
  ) %>%
  mutate(
    magnitude = case_when(
      effsize < 0.2 ~ "Negligible",
      effsize < 0.5 ~ "Small",
      effsize < 0.8 ~ "Medium",
      effsize < 1.0 ~ "Large",
      TRUE ~ "Very large"
...
normTest <-
  stats_data %>%
  select(user_ID,concept, difference) %>%
  pivot_wider(names_from = concept, values_from = difference) %>%
  select(-user_ID) %>%
  stat.desc(norm = T) %>%
  rownames_to_column(var = "statistic") %>%
  filter(statistic %in% c("normtest.W","normtest.p")) %>%
  pivot_longer(cols = -statistic, names_to = "concept", values_to = "value") %>% 
  pivot_wider(names_from = statistic, values_from = value)
```

```{r combine stats tables}
#combine the tables
combined_stats <- 
  left_join(
    #join descriptives and test results
    stats_data %>%
      #calculate the mean and standard deviation for each concept
      group_by(concept) %>%
      summarise(
  ...
  #and join the effect sizes
  left_join(.,effect_sizes) 
# %>%
#   #and join the normality test
#   left_join(.,normTest)
...
combined_stats %<>%
  ...
  #   normtest.p = 
  #     case_when(
  #       normtest.p < 0.001 ~ "<0.001",
  #       TRUE ~ (round(normtest.p,3) %>% as.character())
  #     )
  )
...
#   ) %>%
#   mutate(
#     statistic = paste0(t, " (", parameter.df, ")")
#   )
...
combined_stats %<>% 
  relocate("Method", .after = "SD")

combined_stats_GT <-
combined_stats %>%
  gt(rowname_col = "concept") %>%
  #add spanner
  tab_spanner(
    label = "Estimate",
    columns = c("Mean","SD")
  ...
  tab_spanner(
    ...
    columns = c("Method","t","df","p")
  ) %>%
  tab_spanner(
    label = "Effect size",
    columns = c("Cohen-D","Magnitude")
  ) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = list(cells_column_labels(everything()),cells_row_groups(everything()))
  ) %>%
  #add footnote for Estimate spanner
  tab_footnote(
    footnote = md("When pre- and post-study measures are recorded, these are the mean and standard deviation of the difference between pre- and post-study scores. When only post-study measures are recorded, these are the mean and standard deviation of the post-study scores compared to a population mean of 4 (the mid-way Likert value)."),
    locations = cells_column_spanners(1)
  )
  #add footnote 

#save the table
gt::gtsave(combined_stats_GT, here("output","tables","combined_stats_GT.png"))
gt::gtsave(combined_stats_GT, here("output","tables","combined_stats_GT.docx"))


```

```{r concept by engagement}
#plot the differences in knowledge, self-efficacy and intention against the number of receipts per user
stats_data %>%
  left_join(.,survey_data_all_wide %>% select(user_ID,n_receipts_user, n_receipts_hh)) %>%
  # mutate(difference = ifelse(concept == "Knowledge score", difference/100*7, difference)) %>%
  ggplot(aes(y=difference, x=n_receipts_user, color = concept)) +
  geom_point() +
  geom_smooth(method = "lm") + 
  facet_wrap(~concept, scales = "free") +
  theme_classic() + 
  labs(x = "Number of receipts per user", y = "Difference", color = "")

ggsave(here("output","plots","concept_by_receipt_n.png"), width = 6)
```
